...
public static class EvdevParser
{
        ...
        CancellationToken ct)
    {
        try
        {
            ...
            {
                ReadResult result = await reader.ReadAtLeastAsync(EventSize, ct);
                ReadOnlySequence<byte> buffer = result.Buffer;

                while (buffer.Length >= EventSize)
                {
                    EvdevEvent ev = Parse(buffer.Slice(0, EventSize));
    ...
    private static EvdevEvent Parse(ReadOnlySequence<byte> slice)
    {
        ...
        ushort type  = BinaryPrimitives.ReadUInt16LittleEndian(span[8..]);
        ushort code  = BinaryPrimitives.ReadUInt16LittleEndian(span[10..]);
        int    value = BinaryPrimitives.ReadInt32LittleEndian(span[12..]);
